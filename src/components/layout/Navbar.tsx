...
import { Bell, Search, User, Menu, X, Briefcase, Calendar, ActivitySquare, BarChart2 } from 'lucide-react';
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center space-x-4">
          ...
          <Link to="/profile">
            <Button variant="ghost" size="icon" className={cn("text-muted-foreground hover:text-foreground", isActive('/profile') && "text-foreground bg-accent")}>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-[60px] bg-background z-40 animate-fade-in">
          <div className="p-6 flex flex-col space-y-6">
            <Link 
              to="/" 
              className={cn("text-lg font-medium", isActive('/') && "text-primary")}
            ...
            <Link 
              to="/network" 
              className={cn("text-lg font-medium", isActive('/network') && "text-primary")}
              onClick={() => setIsMobileMenuOpen(false)}
            >
              Network
            </Link>
            <Link 
              to="/jobs" 
              className={cn("text-lg font-medium", isActive('/jobs') && "text-primary")}
              onClick={() => setIsMobileMenuOpen(false)}
            >
              Jobs
            </Link>
            <Link 
              to="/events" 
              className={cn("text-lg font-medium", isActive('/events') && "text-primary")}
              onClick={() => setIsMobileMenuOpen(false)}
            >
              Events
            </Link>
            <Link 
              to="/feed" 
              className={cn("text-lg font-medium", isActive('/feed') && "text-primary")}
              onClick={() => setIsMobileMenuOpen(false)}
            >
              Feed
            </Link>
            <Link 
              to="/analyst" 
              className={cn("text-lg font-medium", isActive('/analyst') && "text-primary")}
              onClick={() => setIsMobileMenuOpen(false)}
            >
              Analyst
            </Link>
            <Link 
              to="/profile" 
              className={cn("text-lg font-medium", isActive('/profile') && "text-primary")}
              onClick={() => setIsMobileMenuOpen(false)}
            >
              Profile
            </Link>
